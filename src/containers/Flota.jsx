  ...
  useEffect(() => {
    const fetchAll = async () => {
      ...
      try {
        ...
        setTractores(tractores.filter(tr => tr.estado === true || tr.estado === 1).length);
        setFurgones(furgones.filter(fg => fg.estado === true || fg.estado === 1).length);
        setUtilitarios(utilitarios.filter(ut => ut.estado === true || ut.estado === 1).length);
      } catch (error) {
