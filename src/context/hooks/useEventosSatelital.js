...
export default function useEventosSatelital() {
  const {
    personas = [],
    empresas = [],
    tractores = [],
  ...
  console.log("[Context] Inicio de filtrado de eventos (useEventosSatlital)");

  const {data: eventosOrdenados } = useEventos();

  const AREA = "satelital";

  const eventosSatelital = useMemo(() => {
  ...
  console.log("[Context] Filtrado finalizado (useEventosSatelital) ✓✓");
  return { eventosSatelital, loading };
}
